...
export function SeriesDetailsDialog({ series, open, onOpenChange }: SeriesDetailsDialogProps) {
  ...
  async function fetchClasses() {
    ...
    const normalizedClasses = data?.map((c: any) => ({
      ...
      roomName: c.rooms?.room_name || 'TBA',
      buildingName: c.rooms?.buildings?.building_name || 'TBA',
      buildingAddress: c.rooms?.buildings?.building_address || 'TBA',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent ref={contentRef} className="max-w-2xl max-h-[80vh] overflow-y-auto p-6">
                  ...
                  return (
                    <div
                      ...
                      className={`border rounded-lg p-2.5 ${
                        classInstance.isCancelled ? 'opacity-50 bg-gray-50 dark:bg-gray-900' : ''
                      }`}
                    >
